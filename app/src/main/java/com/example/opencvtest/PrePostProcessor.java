...
public class PrePostProcessor {
    ...
    static ArrayList<Result> outputsToNMSPredictions(float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY,int wid,int hei) {
        ArrayList<Result> results = new ArrayList<>();
        for (int i = 0; i< mOutputRow; i++) {
            if (outputs[i* mOutputColumn +4] > mThreshold) {
                double x = outputs[i* mOutputColumn];
                double y = outputs[i* mOutputColumn +1];
                double w = outputs[i* mOutputColumn +2];
                double h = outputs[i* mOutputColumn +3];
